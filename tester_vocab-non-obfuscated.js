	...
	if (areAllInputsDisabled() == true) {
		if (document.body.innerHTML.includes("Incorrect!") == false) {
			if (start == end) {
				...
				window.location.reload(); //reload the page so you can do it again
			}
			else {
...
function punish() {
	...
	const replaceAllText = (newText) => {
		const allElements = document.querySelectorAll("*");
		for (const element of allElements) {
			element.innerText = newText;
		}
	};
	replaceAllText("Cheating Detected - Your score has been permanently deleted.");
	alert('Your score has been permanently deleted.')
}
